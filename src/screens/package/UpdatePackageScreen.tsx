...
import { getKnowledgeCategories, updateServicePackage } from "@/src/services/api";
...
export default function UpdatePackageScreen() {
    ...
    const id = useState(packageId || "");
    const [title, setTitle] = useState(packageTitle || "");
    const [categoryLabel, setCategoryLabel] = useState("");
    const [selectedCategoryId, setSelectedCategoryId] = useState<string>("");
    const [categories, setCategories] = useState<any[]>(category ||[]);

    const [priceRaw, setPriceRaw] = useState(packagePrice || "");
    const [price, setPrice] = useState(Number(priceRaw).toLocaleString("vi-VN") || "");
    ...
    const handleSubmit = async () => {
        if (!title || !content || !priceRaw || !durationMinutes || !selectedCategoryId || !image || !commissionRate) {
                ...
                <ScrollView style={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                    <View style={styles.card}>
                        ...
                        <TouchableOpacity style={styles.imageUpload} onPress={pickImage}>
                            {image || imageUrl ? (
                                <Image source={{ uri: image?.uri || imageUrl }} style={styles.imagePreview} />
                            ) : (
